...
import pageIcon from "../src/index";
import { isIconValid, saveToFile } from "./helpers";
...
describe("Page Icon", function () {
  ...
  it("Can download all icons", function (done) {
    const downloadTests = SITE_URLS.map(function (siteUrl): Promise<void> {
...
function iconTypeTest(ext: string, callback: () => void) {
